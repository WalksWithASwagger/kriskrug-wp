...
# kk-marquee-board plugin assets (Tier 3): the CPT pages enqueue these. Generated from
# render.py so the board CSS/JS has a single source of truth across theme + plugin + dist.
PLUGIN_ASSETS = ROOT / "plugins" / "kk-marquee-board" / "assets"
...
def write_plugin_assets():
    """Generate the kk-marquee-board plugin's board CSS + JS from render.py (single source)."""
    if not PLUGIN_ASSETS.parent.exists():
        return  # plugin not present in this checkout — skip silently
    PLUGIN_ASSETS.mkdir(parents=True, exist_ok=True)
    header = "/* GENERATED by scripts/marquee/build.py from render.py — do not edit by hand. */\n"
    (PLUGIN_ASSETS / "marquee.css").write_text(header + TOKENS_WP + BOARD_RULES, encoding="utf-8")
    (PLUGIN_ASSETS / "marquee.js").write_text(theme_js(), encoding="utf-8")
    print(f"  plugin → kk-marquee-board/assets/marquee.css + marquee.js")
