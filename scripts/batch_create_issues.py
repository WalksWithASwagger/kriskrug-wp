...
def validate_issue(issue: Any) -> List[str]:
    """Validate an issue payload. Returns list of errors."""
    errors = []
...
def create_issue(
    issue: Dict[str, Any], repo: str = None, dry_run: bool = False
) -> Dict[str, Any]:
    ...
    try:
        ...
        return {"title": title, "success": False, "error": e.stderr}
...
def main():
    ...
    parser.add_argument(
        "--repo", help="Repository (owner/name) - uses current repo if not specified"
    )
    parser.add_argument(
        "--dry-run", action="store_true", help="Preview without creating issues"
    )
    ...
    if not format_type:
        if file_path.suffix.lower() == ".json":
            ...
            print("Error: Cannot auto-detect format. Use --format", file=sys.stderr)
            sys.exit(1)
    ...
    if not isinstance(issues, list):
        print("Error: Issues must be an array", file=sys.stderr)
        sys.exit(1)
    ...
    validation_errors = []
    if not issues:
        validation_errors.append("No issues found in file")
    ...
    for i, issue in enumerate(issues, 1):
        ...
        if result.get("success", True):  # dry-run doesn't set success key
            ...
            failed += 1
            print(
                f"  ✗ Failed: {result.get('error', 'Unknown error')}", file=sys.stderr
            )
