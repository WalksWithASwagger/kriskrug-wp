...
| Live readback | `https://kriskrug.co`, logged out, 2026-08-02, 16 routes fetched by HTTP GET |
...
| Status | **NOT DECIDED.** Section 10 is the decision block. |
...
| `assets/css/09-late.css` | 1,300 | 23 | 0 | 0 | 0 | 0 | 0 | 0 | 0 |
...
Three of the 176 sit inside comments (`revive-port.css:1167`, `09-late.css:11`, `09-late.css:12`), so **173 are real declarations, 172 of them front-end**. That matches the ratchet in `.css-budget.json` exactly.

### 1.1 The trend, which is the part that matters

| | 2026-07-19 (`33887e7c`) | 2026-07-25 (`0064b4e`, 1.4.3) | 2026-08-02 (`dd87d4a`, 1.5.8) |
|---|---:|---:|---:|
| CSS files | 5 | 6 | 8 |
| Front-end lines | 6,219 | 7,281 | **7,903** |
| Front-end `!important` (code only) | 79 | 160 | **172** |

The first two columns are quoted from `AURORA-STYLESHEET-REBUILD-PLAN.md` §1.1 (which documents its own reproduction commands). The third is this run.

Since KK chose Path A on 2026-07-24 and the safety net shipped, front-end CSS grew **+8.5% in lines** and **+7.5% in `!important`**. The plan's target for the same metrics is 3,000 lines and 5 `!important`.

Where the +12 came from is in `.css-budget.json` waivers, and it is worth reading out loud, because it is the whole argument in miniature:

- **#618** (Aurora 1.5.7, full-bleed portrait hero, already deployed live): +90 lines, **+13 `!important`**. Waiver reason, verbatim from the file: *"The `!important` block is the light-ink-over-dark-photo override the cream-paper system forces on hero h1/dek/kicker/ghost-button."* And: *"Freeze is retroactive bookkeeping: the merge landed without a ratchet gate and left main red, blocking every theme PR."*
- **#596** (Aurora 1.5.8, testimonials): +268 lines, zero new `!important`.

Read that first waiver again. The ratchet from step 0 worked exactly as designed: it went red. It was then waived after the fact because the merge had already landed and main was blocked. That is the incremental failure mode reproducing itself **inside** the Path A rollout, because the rollout stopped after step 2 while feature work kept shipping.

**Nine days of Path A being the plan of record, with the plan paused, cost 12 more `!important` and 622 more lines than the day it was chosen.**

---

## 2. Does the 139 figure from #256 still hold? No. It is now 154.

#256's audit deliverable (`docs/current-state/CSS-DEADCODE-OVERLAP-AUDIT.md:263`) says:

> Legitimate and out of scope for #423 removal: 14 (reduced-motion x8, focus outlines x4, print x2). **Cascade-war casualties: 139** (57 in `style.css`'s two hardening blocks and all 82 in `revive-port.css`).

Re-classified on 1.5.8 with a brace-tracking pass over the four `!important`-carrying sheets (comments stripped, at-rule context tracked properly):

| Category | #256 (at 1.4.3) | Now (1.5.8) | Δ |
|---|---:|---:|---:|
| Legitimate: `prefers-reduced-motion`, `print`, `forced-colors`, `prefers-contrast` | 14 (combined with focus) | 10 | |
| Focus / outline declarations | (in the 14) | 8 | |
| **Cascade war** | **139** | **154** | **+15** |
| Front-end total (code only) | 160 | **172** | +12 |

Per file: `style.css` 72 total (6 legitimate, 4 focus, **62 cascade war**), `revive-port.css` 94 total (0 legitimate, 4 focus, **90 cascade war**), `typography-refined.css` 6 total (4 legitimate, **2 cascade war**).

**The 139 figure no longer holds. It is 154, up 10.8%.** The direction is the finding, not the digit.
...
## 4. The full CSS surface on a live page: seven layers, one of which the theme cannot see

Measured on `/about/`, logged out, 2026-08-02, in document order:

| # | Surface | Bytes | `!important` | Owner |
|---|---|---:|---:|---|
| 1 | Jetpack Boost concatenated bundle (`s5102.pcdn.co/wp-content/boost-cache/static/*.min.css`), the **only** `<link rel=stylesheet>` on the page | ~138 KB | ~159 | all 7 front-end theme sheets, minified into one file |
| 2 | 8 WP core block inline styles | ~5.8 KB | 2 | WordPress |
...
| 6 | Jetpack Boost critical CSS (`/` and `/blog/` only) | 7,474 on `/`, 15,332 on `/blog/` | 1 | Jetpack |
...
- **Surface 3 prints after surface 1.** `theme.json`'s generated CSS lands after the entire theme bundle and is unlayered. At equal specificity it wins. That single ordering fact is the mechanical reason most of those 154 cascade-war `!important` declarations exist: the theme is fighting its own token layer from one document position too early. Step 2 (#474) already shipped the answer to this, `09-late.css`, and it is currently **empty by design** (23 lines, all comment). Nothing has needed it yet, which is a good sign for the mechanism and no evidence at all about the migration.
...
- The debt is measurably still growing under every non-rebuild regime tried so far: 79 to 160 to 172 `!important`, 6,219 to 7,281 to 7,903 lines. Two separate measurement windows, same direction.
...
- It has been tried, measured, and it lost. Twice. 79 to 160 across the 1.4.x Revive port while B was nominally in force, then 160 to 172 across 1.5.7 and 1.5.8 while A was nominally in force but paused. In both windows a shipping feature added `!important` faster than cleanup removed it.
...
Path A is the recorded decision, steps 0 to 2 shipped, steps 3 to 8 are stalled, and feature work continues under de facto Path B rules with retroactive waivers. This is the **worst** of both: the coordination cost of A (six issues blocked behind one) plus the drift of B (+12 `!important` in nine days) plus a growing gap between the plan doc and reality.
...
| Rollback story | Re-upload previous zip per step; content restore for #480 and #481 | n/a | n/a |
...
| Cost of being wrong | Weeks of Track B time with no user benefit; a bad step is one zip re-upload away from reverted | Debt keeps compounding at roughly +1.3 `!important` per day | both |
...
**Not decided. Nothing below has been actioned.**

Note before you read it: you already answered the headline question on 2026-07-24 and reconfirmed it on 2026-07-25. This block is not asking you to re-litigate that. It is asking about the four things that have changed since, and about the sequencing that has six issues stuck behind one.
...
| Historical columns in 1.1 and 2 | `AURORA-STYLESHEET-REBUILD-PLAN.md` §1.1, `CSS-DEADCODE-OVERLAP-AUDIT.md:263` |
...
- The `~138 KB / ~159 !important` figure for the Jetpack Boost bundle in section 4 is carried over from `AURORA-STYLESHEET-REBUILD-PLAN.md` §1.6 (measured 2026-07-25 at 1.4.3), not re-measured today. The bundle hash has changed since. Treat it as approximate.
