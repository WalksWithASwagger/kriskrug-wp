...
- Private, create-only WordPress drafts are temporarily unblocked for review when dry-run, slug, category, and draft-status checks pass. Public publish, existing-content updates, destructive cleanup, and plugin/theme/schema/robots changes still require backup and restore proof.
...
Goal: make Track A public production work safe again.
...
This blocks `/llms.txt`, robots, homepage H1/alt text, Work OG/slug cleanup, sidebar promo deploy, public publish, existing-content updates, destructive cleanup, media-heavy imports, bulk writes, and any Notion-to-WP `--update` run.

It does not block private, create-only WordPress drafts for review when the publisher uses dry-run first, verifies the slug is new, keeps status as `draft`, and records the WP edit URL.
...
3. Batch small `swarm-ready` issues (`#8`, `#9`, `#10`, `#36`, `#43`, `#48`) only after confirming none require public publish, existing-content updates, destructive cleanup, plugin/theme/schema/robots changes, media-heavy imports, bulk writes, or `--update` before the backup gate.
...
6. Keep blocked lanes (`#23`, `#75`, public publish, existing-content updates, destructive cleanup, plugin/theme/schema/robots changes, media-heavy imports, bulk writes, Aurora activation/staging promotion) out of autonomous implementation until the human or backup gate clears them. Private create-only draft review can proceed with dry-run and slug checks.

## Stop Rules

1. No public publish, existing-content updates, destructive cleanup, plugin/theme/schema/robots changes, media-heavy imports, bulk writes, or Notion-to-WP `--update` before backup/restore proof. Private create-only drafts may be created for review after dry-run, slug, category, and draft-status checks.
