...
`PRACTICE_PORT` changes the manual preview port (default 9832). Browser tests use 9833 and close their fixture. `PRACTICE_CHROME` can select a local Chrome executable; CI installs pinned Playwright Chromium. `PRACTICE_EVIDENCE` selects screenshot/report output (default `/tmp/kk-practice-evidence`). Evidence uses synthetic answers only. Do not submit participant text in issues, logs or screenshots.
