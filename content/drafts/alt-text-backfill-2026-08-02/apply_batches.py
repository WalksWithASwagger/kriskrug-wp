...
def main() -> int:
    ...
    run_dir = SNAPSHOT_ROOT / f"{stamp}-{args.batch}-{'apply' if args.apply else 'dry'}"
    ...
    for item in bad.get("items", []) + [
        ...
        if str(item.get("status", "")).startswith(("REFUSED", "CONFLICT", "WRITTEN-")):
