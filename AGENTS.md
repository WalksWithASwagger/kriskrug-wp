...
3. [`docs/current-state/WORK-PLAN-2026-08-23.md`](docs/current-state/WORK-PLAN-2026-08-23.md) — **day runbook** (Aurora 1.6.9 live/repo parity; hub packs #829-#832 next; merge lane is admin-override only; supersedes 2026-08-17)
...
- **Content/docs-only PRs:** agents may merge these directly. `main` requires **0 approving reviews** as of 2026-08-23 (KK is the only collaborator, so the old 1-review rule was unsatisfiable and every merge had to bypass it with `--admin`, which skipped CI enforcement too). What is still required: **`Test PR / summary` green** and the branch **up to date with `main`** (`strict: true`). Merge with `gh pr merge <n> --squash --delete-branch` - **no `--admin`**, so CI is genuinely enforced. Reach for `--admin` only when KK asks you to override a red or stale check. Force pushes and branch deletion on `main` stay blocked. The `agent-safe-merge` workflow was deleted after 33 runs and 0 successes; its label does nothing. `main` requires 1 approving review and GitHub blocks self-approval, so a PR authored by `WalksWithASwagger` cannot be approved by that same account. Today the only working path is KK's admin override (`gh pr merge <n> --squash --admin`, `enforce_admins` is `false`), and normal approve+merge for Dependabot PRs. Status and the real fix: [`docs/current-state/AGENT-MERGE-PATH-2026-07-26.md`](docs/current-state/AGENT-MERGE-PATH-2026-07-26.md).
...
- **GitHub merge from Cloud:** set Cursor Cloud secret `GH_TOKEN` to a classic PAT with `repo` scope owned by a write-access account that is **not** the PR author (same value as Actions secret `AGENT_MERGE_TOKEN`, which is currently unset). A PAT owned by the PR author still cannot approve, so it does not unblock the queue. Without a valid second-account token, `gh pr review --approve` / `gh pr merge` fail under branch protection even when CI is green. Details: [`docs/current-state/AGENT-MERGE-PATH-2026-07-26.md`](docs/current-state/AGENT-MERGE-PATH-2026-07-26.md).
