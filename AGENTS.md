...
The operations + content hub for [kriskrug.co](https://kriskrug.co/) — a Pagely-hosted WordPress site running the Aurora theme (`kk-aurora`). **Live** theme (`style.css` Version) as of 2026-07-27 public readback: **1.5.0**. **Repo** `main` theme line: **1.5.0** — reconciled to live on 2026-07-27 when PR #493 (`theme/474-cascade-layers-scaffold`, the cascade `@layer` + token scaffold) merged; #474/#494/#545 closed. Repo and live are in sync. WordPress publicly reports **7.0.2**. How verified: `curl -s https://kriskrug.co/wp-content/themes/kk-aurora/style.css | grep -i version` on 2026-07-27 returned `Version: 1.5.0`, matching `main`. The repo is **adjacent to** the live site, not a mirror of it. `main` contains the canonical tracked theme line, plus content/ops tooling and docs. Custom repo-side WP code includes `inc/digital-composting.php` and `plugins/kk-sidebar-promos/` (deploy only with an explicit rollback path and KK approval).
...
- Live Aurora and repo Aurora drift in **either** direction. As of 2026-07-27 they are **in sync** at **1.5.0** (repo `main` reconciled to live when PR #493 merged; #545 closed). Historically the drift has gone both ways: live has been ahead of `main` (the 1.5.0 deploy predated the merge) and behind it. Do not treat `theme/kk-aurora/style.css` Version as proof of production without a public `style.css` readback, and do not assume the direction of any future drift. Readback, don't assume, in either direction.
