...
The operations + content hub for [kriskrug.co](https://kriskrug.co/) — a Pagely-hosted WordPress site running the Aurora theme (`kk-aurora`). **Live** theme (`style.css` Version) as of 2026-08-02 public readback: **1.5.7**. **Repo** `main` theme line: **1.5.8**. Repo is **one patch ahead of live**: 1.5.7 was SFTP-deployed on 2026-08-01 (the full-bleed portrait hero, PR #618) and `main` then moved to 1.5.8 with the `aurora-tstm` testimonials CSS (PR #629), which is **built but not deployed** — its live deploy is gated on the #601 pixel gate. WordPress publicly reports **7.0.2**. How verified: `curl -s https://kriskrug.co/wp-content/themes/kk-aurora/style.css | grep -i version` on 2026-08-02 returned `Version: 1.5.7` against `main` at 1.5.8. The repo is **adjacent to** the live site, not a mirror of it. `main` contains the canonical tracked theme line, plus content/ops tooling and docs. Custom repo-side WP code includes `inc/digital-composting.php` and `plugins/kk-sidebar-promos/` (deploy only with an explicit rollback path and KK approval).
...
- Live Aurora and repo Aurora drift in **either** direction. As of 2026-08-02 live is **1.5.7** and repo `main` is **1.5.8** (repo ahead by the undeployed `aurora-tstm` CSS, gated on #601). Historically the drift has gone both ways: on 2026-07-27 they were in sync at 1.5.0, and on 2026-08-01 live ran *ahead* of `main` because the 1.5.7 hero was SFTP-deployed before PR #618 merged. Do not treat `theme/kk-aurora/style.css` Version as proof of production without a public `style.css` readback, and do not assume the direction of any future drift. Readback, don't assume, in either direction.

---

**Last verified:** 2026-08-02 (live WP **7.0.2**; Aurora live **1.5.7**, repo `main` **1.5.8** with the gap gated on #601; front door is CURRENT-STATE / WORK-PLAN / MASTER-PLAN 2026-07-30; Cloud secrets may still be unset in long-lived pods). If you're reading this much later and the repo has drifted, run `make morning-truth` (or `make status-readonly`) and treat the newest committed `docs/current-state/reports/morning-truth-*.md` as the source of truth.
