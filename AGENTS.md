...
The operations + content hub for [kriskrug.co](https://kriskrug.co/) — a Pagely-hosted WordPress site running the Aurora theme (`kk-aurora`). **Live** theme (`style.css` Version) as of 2026-08-09 public readback: **1.5.9**. **Repo** `main` theme line: **1.6.0**. Repo is **three versions ahead of live**: 1.5.10 (a11y focus-visible states, #676), 1.5.11 (primitives layer, #680), and 1.6.0 (dead-CSS purge, #681) are all built but not deployed. WordPress publicly reports **7.0.3** (auto-updated from 7.0.2). How verified: `curl -s https://kriskrug.co/wp-content/themes/kk-aurora/style.css | grep -i version` against `theme/kk-aurora/style.css`. Do not treat the repo `style.css` version as proof of production. Read back the public file, in either direction. The repo is **adjacent to** the live site, not a mirror of it. `main` contains the canonical tracked theme line, plus content/ops tooling and docs. Custom repo-side WP code includes `inc/digital-composting.php` and `plugins/kk-sidebar-promos/` (deploy only with an explicit rollback path and KK approval).
...
- Live Aurora and repo Aurora drift in **either** direction. As of 2026-08-09 live is **1.5.9** and repo `main` is **1.6.0** (three undeployed versions, deploy window gated on KK approval + pixel gate). Historically the drift has gone both ways: on 2026-07-27 they were in sync at 1.5.0, and on 2026-08-01 live ran *ahead* of `main` because the 1.5.7 hero was SFTP-deployed before PR #618 merged. Do not treat `theme/kk-aurora/style.css` Version as proof of production without a public `style.css` readback, and do not assume the direction of any future drift. Readback, don't assume, in either direction.

---

**Last verified:** 2026-08-09 (live WP **7.0.3**; Aurora live **1.5.9**, repo `main` **1.6.0** with a three-version deploy gap awaiting the pixel-gated window; front door is CURRENT-STATE 2026-07-30 / WORK-PLAN 2026-08-09 / MASTER-PLAN 2026-07-30; Cloud secrets may still be unset in long-lived pods). If you're reading this much later and the repo has drifted, run `make morning-truth` (or `make status-readonly`) and treat the newest committed `docs/current-state/reports/morning-truth-*.md` as the source of truth.
