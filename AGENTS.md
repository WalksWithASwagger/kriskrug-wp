...
The operations + content hub for [kriskrug.co](https://kriskrug.co/) — a Pagely-hosted WordPress site running the Aurora theme (`kk-aurora`). **Live** theme as of 2026-07-25 public readback: **1.4.3**. **Repo** `main` theme line: **1.4.4** (a11y contrast + skip-link fixes from #464; awaiting deploy). WordPress publicly reports **7.0.2**. The repo is **adjacent to** the live site, not a mirror of it. `main` contains the canonical tracked theme line, plus content/ops tooling and docs. Custom repo-side WP code includes `inc/digital-composting.php` and `plugins/kk-sidebar-promos/` (deploy only with an explicit rollback path and KK approval).
...
- Live Aurora may lag repo Aurora (2026-07-25: live **1.4.3**, repo **1.4.4** pending deploy). Do not treat `theme/kk-aurora/style.css` Version as proof of production without a public `style.css` readback. Note the lag is *not* guaranteed: on 2026-07-25 live and repo were found byte-identical (md5) across all six CSS files, having been assumed three minor versions apart. Readback, don't assume — in either direction.

---

**Last verified:** 2026-07-25 swarm pass (live WP **7.0.2** confirmed by generator readback, Aurora live **1.4.3** / repo **1.4.4**; live CSS confirmed byte-identical to repo at 1.4.3 before the 1.4.4 merge; Cloud secrets may still be unset in long-lived pods). If you're reading this much later and the repo has drifted, run `make morning-truth` (or `make status-readonly`) and treat the newest committed `docs/current-state/reports/morning-truth-*.md` as the source of truth.
